...
const typeDefs = gql`
  ...
  type Mutation {
    ...
    updateBook(book_id: ID!, title: String, author: String, release_year: Int): Book
...
const resolvers = {
    ...
    Mutation: {
      addBook: async(_, { title, author, release_year }, { dataSources }) => {
        const book = await dataSources.booksAPI.addBook(title, author, release_year);
        return book[0];
      },
      updateBook: async(_, {book_id, title, author, release_year }, { dataSources }) => {
        const book = await dataSources.booksAPI.updateBook(book_id, {title, author, release_year});
        return book[0];
      },
    ...
    Book: {
        async __resolveReference({book_id}, __, { dataSources }){
            return await dataSources.booksAPI.getBooks(book_id);
          },
    }
 }
...
async function start() {
    const { url } = await startStandaloneServer.startStandaloneServer(server, {
        ...
        context: () => ({    
          dataSources: {
            booksAPI: new MyDatabase(knexConfig),
          },
        })
    });
